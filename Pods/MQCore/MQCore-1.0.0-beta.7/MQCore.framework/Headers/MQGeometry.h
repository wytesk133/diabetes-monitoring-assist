 ...
 * @var latitudeDelta the distance in degrees latitude
 * @var longitudeDelta the distance in degrees longigude
 */
typedef struct {
 ...
 * @var center The center coordinate of the span (not origin)
 * @var span The longitudinal and latitudinal dimensions of the region
 */
typedef struct {
...
/** A convenience method for constructing MQBoundingBoxs
 * @param lowerRight The coordinate of the upperRight corner of the box
 * @param upperLeft The coordiante of the lowerRight corner of the box
 */
MQ_EXTERN MQBoundingBox MQBoundingBoxMake(CLLocationCoordinate2D lowerRight, CLLocationCoordinate2D upperLeft);
...
/** A convenience method to determine if the latitude and longitude deltas are both equal to 0.0
 * @param coordinateSpan The coordinate span to check
 */
MQ_EXTERN BOOL MQCoordinateSpanIsZero(MQCoordinateSpan coordinateSpan);
 ...
 * @var x the projected x-axis value, cooresponding to the longitude in a real-world coordinate system
 * @var y the projected y-axis value, cooresponding to the latitude in a real-world coordinate system
 */
typedef struct {
    double x;
    double y;
 ...
 * @var width The width of the map at some scale
 * @var height The height of the map at some scale
 */
typedef struct {
    double width;
    double height;
 ...
 * @var origin The upper left most point of the region
 * @var size The dimension of the map in MQMapPoints
 */
typedef struct {
    MQMapPoint origin;
    MQMapSize size;
...
/** MQZoomLevel In additon size and details, zoom level indicates size of map tile, eg. at 0 each tile
 * covers entire world, 1 1/4 of the world, at 2 1/16, and so on. 
 */
typedef double MQZoomLevel;
 ...
 * @param rect The rect for which the min x is desired
 ...
 * @param rect The rect for which the min y is desired
 ...
 * @param rect The rect for which the max x is desired
 ...
 * @param rect The rect for which the max y is desired
...
// An invalid or unknown coordinate
MQ_EXTERN const CLLocationCoordinate2D MQCoordinateNull;

//  An invalid or unknown region
MQ_EXTERN const MQCoordinateRegion MQCoordinateRegionNull;

/** Returns whether a CLLocationCoordinate2D value is unknown or invalid (equal to MQCoordinateNull)
 * @param coordinate The CLLocationCoordinate2D to test
 */
NS_INLINE BOOL MQCoordinateIsNull(CLLocationCoordinate2D coordinate) {
    return isinf(coordinate.latitude) || isinf(coordinate.longitude);
}

/** Returns whether an MQCoordinateSpan value is unknown or invalid (equal to MQCoordinateSpanNull)
 * @param span The MQCoordinateSpan to test
 */
NS_INLINE BOOL MQCoordinateSpanIsNull(MQCoordinateSpan span) {
    return isinf(span.latitudeDelta) || isinf(span.longitudeDelta);
}

/** Returns whether an MQCoordinateRegion value is unknown or invalid
 * @param region The MQCoordinateRegion to test
 */
NS_INLINE BOOL MQCoordinateRegionIsNull(MQCoordinateRegion region) {
    return MQCoordinateIsNull(region.center) && MQCoordinateSpanIsNull(region.span);
}
